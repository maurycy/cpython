...
// can be included from the bundled mimalloc translation unit.
...
// pre-configuration; the inline helpers below only read them so there is no
// lazy-init data race in free-threaded builds.
extern int _Py_mimalloc_reusable_enabled;
extern size_t _Py_mimalloc_reusable_pagesize;

static inline int
_PyDarwinVM_ReusableEnabled(void)
{
    return _Py_mimalloc_reusable_enabled;
}
...
{
    size_t ps = _Py_mimalloc_reusable_pagesize;
    if (ps == 0) {
        *aligned = ptr;
        return 0;
    }
...
// errno from madvise() so the caller can fall back to MADV_DONTNEED.
static inline int
